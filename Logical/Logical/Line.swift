...
class Line: NSObject {
    ...
    func updateWithTouch(touch: UITouch, inView view: UIView) -> (Bool, CGRect) {
        if  let estimationUpdateIndex = touch.estimationUpdateIndex,
            ...
            var rect = updateRectForExistingPoint(point)
                let didUpdate = point.updateWithTouch(touch, inView: view)
            if didUpdate {
    ...
    func addPointOfType(pointType: LinePoint.PointType, forTouch touch: UITouch, inView view: UIView) -> CGRect {
        let previousPoint = points.last
        let previousSequenceNumber = previousPoint?.sequenceNumber ?? -1
        let point = LinePoint(touch: touch, view: view, sequenceNumber: previousSequenceNumber + 1, pointType:pointType)
...
class LinePoint: NSObject  {
    ...
    init(touch: UITouch, view: UIView, sequenceNumber: Int, pointType: PointType) {
...
//        let view = touch.view
    ...
    func updateWithTouch(touch: UITouch, inView view: UIView) -> Bool {
